...
function ProductDetailOne(props) {
  ...
  useEffect(() => {
    ...
    const images = product.props_imgs && product.props_imgs.prop_img;
    const sku = product.skus.sku;
    setSku(sku);
    setEnt(entries);
    setImg(images);
    setPrice(product.price);
    setStock(product.num);
    setData(product);
    quantityInputs();
  }, []);
          ...
          {0 === product.stock ? (
            ...
            <div className="product-price">
              {product && product.price ? (
                <Price id={id} data={data} outPrice={price} />
              ) : (
          ...
          <div className="details-filter-row details-row-size">
            ...
            <div className="product-details-quantity">
              <input
                ...
                required
              />
              {product && product.skus.sku.length ? (
                <Stock id={id} data={data} entries={ent} />
              ) : (
                <p>Stock:{stock}</p>
              )}
            </div>
